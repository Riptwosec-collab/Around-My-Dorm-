...
export function GoogleCloudAutoEnrichment({
  ...
  async function runBulk() {
    ...
    if (!admin.admin) {
      ...
      setMessage(
        language === "en"
          ? "Authorized admin sign-in is required before sending bulk Google requests."
          : "ต้องเข้าสู่ระบบด้วยบัญชี Admin ที่ได้รับสิทธิ์ก่อนยิง Google แบบ Bulk",
      );
      return;
    }
      ...
      if (next.stoppedBySystemicError) {
        setMessage(
          language === "en"
            ...
            : `หยุดหลังพบ Google error ระดับระบบครั้งแรก เพื่อไม่ยิงซ้ำทั้งชุด • ${next.stoppedReason || next.lastError || "ดู Diagnostics"}`,
        );
      } else if (next.stoppedByLimit) {
        setMessage(
          language === "en"
            ? `${next.stoppedReason || "Safety limit reached."} Completed shops are saved. Press again to continue the remaining shops.`
            : `${next.stoppedReason || "ถึง Safety Limit"} • ร้านที่เสร็จแล้วถูกบันทึกไว้ กดอีกครั้งเพื่อทำร้านที่เหลือต่อ`,
        );
  ...
  return (
    <>
      ...
      <section data-testid="google-cloud-auto-enrichment" className="amd-glass amd-card mt-4 overflow-hidden p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded-full border px-2 py-1 text-[8px] font-extrabold ${admin.admin ? "border-emerald-300/20 bg-emerald-300/[0.07] text-emerald-200" : "border-amber-300/20 bg-amber-300/[0.07] text-amber-100"}`}>
                {admin.admin ? "ADMIN READY" : "ADMIN LOCKED"}
              </span>
            </div>
            <p className="mt-1 text-[9px] leading-5 text-[var(--amd-text-2)]">
              {language === "en"
                ? `One confirmed admin click checks every shop, links clear Google Place IDs, fetches details, and stores a ${GOOGLE_CLOUD_CACHE_TTL_DAYS}-day shared Supabase cache. Normal browsing never calls Google.`
                : `Admin กดยืนยันครั้งเดียวเพื่อไล่ทุกร้าน → หา Google Place ID → ดึงรายละเอียด → เก็บ Shared Supabase Cache ${GOOGLE_CLOUD_CACHE_TTL_DAYS} วัน • เปิดเว็บปกติไม่ยิง Google`}
            </p>
          </div>
        ...
        {!admin.admin && (
          <div className="mt-3 flex items-start gap-2 rounded-2xl border border-amber-300/15 bg-amber-300/[0.05] p-3 text-[9px] leading-5 text-amber-100">
            <LockKeyhole className="mt-0.5 h-4 w-4 shrink-0" />
            <span>{language === "en" ? "Bulk Google requests are locked until a non-anonymous account with app_metadata.amd_admin=true signs in." : "ล็อก Bulk Google ไว้จนกว่าจะเข้าสู่ระบบด้วยบัญชีจริงที่มี app_metadata.amd_admin=true"}</span>
          </div>
        )}
        ...
        <div className="mt-3 rounded-2xl border border-white/[0.06] bg-black/10 p-3 text-[8px] leading-5 text-white/45">
          <div className="flex items-center gap-2">
            ...
            <strong className="text-white/70">Shared cached place data • admin-only refresh</strong>
          </div>
          <p className="mt-1">
            {language === "en"
              ? "Cached place data remains readable without login, but cost-bearing bulk refresh requires explicit admin authorization."
              : "ข้อมูล Cache อ่านได้โดยไม่ต้อง Login แต่การ Refresh แบบ Bulk ที่มีค่า API ต้องผ่านสิทธิ์ Admin ก่อน"}
          </p>
        ...
        {requestRisk !== "normal" && (
          <div className={`mt-3 flex items-start gap-2 rounded-2xl border p-3 text-[9px] leading-5 ${requestRisk === "limit" ? "border-rose-300/15 bg-rose-300/[0.05] text-rose-100" : "border-amber-300/15 bg-amber-300/[0.05] text-amber-100"}`}>
            ...
            <span>
              {requestRisk === "limit"
                ? language === "en"
                  ? `Worst-case estimate is ${estimate}, above the ${GOOGLE_BULK_RUN_REQUEST_LIMIT}-request run cap. Progress is saved and another confirmed run can continue.`
                  : `Worst case ${estimate} requests มากกว่า Run Limit ${GOOGLE_BULK_RUN_REQUEST_LIMIT} • ระบบเก็บความคืบหน้า แล้วกดรอบถัดไปเพื่อทำต่อได้`
                : language === "en"
        ...
        <div className="mt-4 flex gap-2">
          <button
            ...
            type="button"
            disabled={running || !apiKey || places.length === 0 || !admin.admin}
          ...
          <div className="fixed inset-0 z-[160] grid place-items-end bg-black/70 p-3 sm:place-items-center">
            ...
            <div className="amd-glass-strong relative w-full max-w-[520px] rounded-[28px] border border-white/[0.08] p-5 shadow-2xl">
              ...
              <div className="mt-4 rounded-2xl border border-white/[0.07] bg-black/20 p-3 text-[9px] leading-5 text-white/55">
                ...
                <p>{language === "en" ? "Fresh cache and existing Place IDs are reused, so repeat runs avoid unnecessary completed-shop requests." : "ร้านที่มี Place ID / Cache สดแล้วจะถูกใช้ซ้ำ เพื่อลด request ที่ไม่จำเป็น"}</p>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2">
                ...
                <button data-testid="confirm-google-cloud-enrich-all" disabled={!admin.admin} type="button" onClick={() => void runBulk()} className="amd-btn amd-btn-primary min-h-11 rounded-xl px-4 text-[9px] font-bold disabled:opacity-40">{language === "en" ? "Send Google Requests" : "ยืนยันและยิง Google API"}</button>
              </div>
            </div>
          </div>
        )}
