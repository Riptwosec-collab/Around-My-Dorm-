...
export function GoogleRouteRefresh({
  ...
  async function refreshRoutes() {
    if (!adminAllowed || !origin || !isUsableHomeOrigin(origin) || running) return;
    setConfirmOpen(false);
    setRunning(true);
    setProgress({ ...EMPTY_PROGRESS, total: eligible.length * 3 });
    setMessage(null);
    try {
      const result = await refreshRoutesForPlaces({
        origin,
        places,
  ...
  return (
    <section className="amd-glass amd-card mt-4 p-4" data-testid="google-route-refresh">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[9px] leading-5 text-white/48">
            {language === "en"
              ? `Walking, driving and two-wheel routes are fetched only after confirmation and cached for ${ROUTE_CACHE_TTL_DAYS} days.`
              : `ดึงเวลาเดิน / รถยนต์ / มอเตอร์ไซค์เฉพาะตอนกดยืนยัน และ Cache ${ROUTE_CACHE_TTL_DAYS} วัน`}
          </p>
        </div>
      ...
      <div className="mt-3 grid grid-cols-3 gap-2">
        ...
        <div className="rounded-xl border border-white/[0.06] bg-black/10 p-3"><p className="text-[8px] text-white/35">{language === "en" ? "API batches" : "API batches"}</p><strong className="text-[18px]">{estimatedRequests}</strong></div>
      </div>
      ...
      {running && (
        <div className="mt-3 rounded-xl border border-violet-300/10 bg-violet-300/[0.04] p-3 text-[9px]">
          ...
          <div className="mt-2 grid grid-cols-4 gap-1 text-center text-[8px] text-white/45">
            <div>API<br/><strong>{progress.requests}</strong></div><div>OK<br/><strong className="text-emerald-200">{progress.success}</strong></div><div>Skip<br/><strong className="text-amber-100">{progress.skipped}</strong></div><div>Fail<br/><strong className="text-rose-200">{progress.failed}</strong></div>
          </div>
        </div>
      )}

      {!running && progress.success > 0 && (
        <div className="mt-3 flex items-start gap-2 rounded-xl border border-emerald-300/10 bg-emerald-300/[0.04] p-3 text-[9px] text-emerald-100"><CheckCircle2 className="h-4 w-4 shrink-0"/><span>{progress.success} route results cached.</span></div>
      )}
      ...
      <button type="button" disabled={!adminAllowed || !verifiedHome || !eligible.length || running} onClick={() => setConfirmOpen(true)} className="amd-btn amd-btn-primary mt-4 min-h-11 w-full rounded-xl px-4 text-[9px] font-bold disabled:opacity-40">
      ...
      {confirmOpen && (
        <div className="mt-3 rounded-2xl border border-violet-300/15 bg-black/20 p-4">
          ...
          <p className="mt-2 text-[8px] leading-4 text-amber-100/80">{language === "en" ? "Walking and two-wheel routes are beta and can omit suitable paths; the app will show the required caution and never invent missing results." : "เส้นทางเดินและสองล้อเป็น Beta • ถ้า Google ไม่คืนเส้นทาง ระบบจะแสดงไม่มีข้อมูลและไม่สร้างเวลาขึ้นเอง"}</p>
