...
export function GoogleMaintenancePanel({ places, language }: { places: Place[]; language: "th" | "en" }) {
  ...
  return (
    <section data-testid="google-api-control-center" className="amd-glass amd-card mt-4 p-4">
      ...
      ].map(([label, value]) => <div key={String(label)} className="rounded-xl bg-white/[0.035] p-2 text-center"><p className="text-[7px] text-white/30">{label}</p><p className="mt-1 text-[14px] font-bold">{value}</p></div>)}</div>{result.cancelled && <p className="mt-2 text-[8px] text-amber-100">Batch cancelled. Completed requests remain completed.</p>}{failedPlaces.length > 0 && <button type="button" disabled={apiControl.locked || Boolean(progress)} onClick={() => void runFailedRetry()} className="amd-chip mt-3 min-h-10 px-3 text-[9px] font-bold text-amber-100">{language === "en" ? `Retry ${failedPlaces.length} Failed Requests` : `Retry ${failedPlaces.length} Requests ที่ล้มเหลว`}</button>}</div>}
